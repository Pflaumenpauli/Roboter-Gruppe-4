...
public class GuidanceAT {
	...
	public enum CurrentStatus {
	 ...
	 private enum SubMissionPARKING {
			/**
			 * drive to slot
			 */
			DRIVE_TO,
	 ...
	 protected static SubMissionPARKING missionThis 		= SubMissionPARKING.DRIVE_TO;
	/**
...
//					monitor.writeGuidanceComment("Guidance_Driving");
					...
					if(stateJudge) 
					{
						...
						{
						    currentStatus = CurrentStatus.PARKING;
						}
					}
...
////////////////////////////////////Park_This////////////////////////////////////
	...
	{   
		...
		missionThis = SubMissionPARKING.DRIVE_TO;							//ParkThis Zusatndautomat initialisieren
...
//	 While action
	{
					...
					(slot.getBackBoundaryPosition().getX()+etaAbstand>pose.getX())&&(slot.getBackBoundaryPosition().getX()-etaAbstand<pose.getX())) 
			{
				...
				if ( (slot.getID()==0)||(slot.getID()==1))
				{
				 ...
				 //wenn Phi<0, dreht der Roboter nach links bis phi gleich 0
				   ...
				   {
					   missionThis = SubMissionPARKING.PARK_IN;	//Zustandsuebrgang
				   }
				}
				if (slot.getID()==2)
				{
		...
		case PARK_IN:
               ...
               //3 Situationen: 1. Slot auf line 0; 2. Slot auf line 1; 3.Slot auf line  4								
				...
				if(((pose.getX()<slot.getBackBoundaryPosition().getX()+0.28+etaAbstand)&&(pose.getX()>slot.getBackBoundaryPosition().getX()+0.28-etaAbstand)&&
						(pose.getY()<slot.getBackBoundaryPosition().getY()-0.28+etaAbstand)&&(pose.getY()>slot.getBackBoundaryPosition().getY()-0.28-etaAbstand))
						||((pose.getX()<slot.getBackBoundaryPosition().getX()+0.28+etaAbstand)&&(pose.getX()>slot.getBackBoundaryPosition().getX()+0.28-etaAbstand)&&
								(pose.getY()<slot.getBackBoundaryPosition().getY()+0.28+etaAbstand)&&(pose.getY()>slot.getBackBoundaryPosition().getY()+0.28-etaAbstand))
						||((pose.getX()<slot.getBackBoundaryPosition().getX()-0.28+etaAbstand)&&(pose.getX()>slot.getBackBoundaryPosition().getX()-0.28-etaAbstand)&&
								(pose.getY()<slot.getBackBoundaryPosition().getY()+0.28+etaAbstand)&&(pose.getY()>slot.getBackBoundaryPosition().getY()+0.28-etaAbstand)))
				{   missionThis = SubMissionPARKING.WINKEL_ADJUSTMENT;
				}
				break;
		...
		case WINKEL_ADJUSTMENT:
			...
			if ( (slot.getID()==0)||(slot.getID()==1))
			{
			 while (pose.getHeading()/Math.PI*180>0)
			   ...
			   if ((pose.getHeading()/Math.PI*180>0-etaWinkel)&&(pose.getHeading()/Math.PI*180>0+etaWinkel))
			   {
				   missionThis = SubMissionPARKING.POSITIONSKORREKTUR;	//Zustandsuebrgang
			   }
			}
			if (slot.getID()==2)
			{
...
//////////////////////////////INACTIVE//////////////////////////////////////////////////
	...
	protected static void showData(INavigation navigation, IPerception perception){
		...
		LCD.drawString("FS (in cm)" + Integer.toString((int)perception.getFrontSideSensorDistance()),0,3);
...
//    	if ( hmi.getMode() == parkingRobot.INxtHmi.Mode.SCOUT ){
//			LCD.drawString("HMI Mode SCOUT", 0, 6);
//		}else if ( hmi.getMode() == parkingRobot.INxtHmi.Mode.PAUSE ){
//			LCD.drawString("HMI Mode PAUSE", 0, 6);
//		}else{
//			LCD.drawString("HMI Mode UNKNOWN", 0, 6);
//		}
	}
}